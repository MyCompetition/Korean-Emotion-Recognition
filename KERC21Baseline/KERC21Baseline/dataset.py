...
class DatasetKERC21(Dataset):
    ...
    def __getitem__(self, index):
        ...
        eeg_ftrs = torch.from_numpy(np.load(f"{self.dataset_dir}/{self.data_type}/{sample_id}_EEG_ftrs.npy")).type(
            torch.FloatTensor)
        eda_ftrs = torch.from_numpy(np.load(f"{self.dataset_dir}/{self.data_type}/{sample_id}_EDA.npy")).type(
            torch.FloatTensor)
        bvp_ftrs = torch.from_numpy(np.load(f"{self.dataset_dir}/{self.data_type}/{sample_id}_BVP.npy")).type(
            torch.FloatTensor)
        personality_ftrs = torch.from_numpy(
            self.personality_traits[self.personality_traits['Id'] == sample_id].iloc[0].values[1:].astype(int)).type(
            torch.LongTensor)
        sample = eeg_ftrs, eda_ftrs, bvp_ftrs, personality_ftrs, self.quadrants.index(
            self.labels[self.labels['Id'] == sample_id]['label'].values[0]) if hasattr(self,
                                                                                       'labels') else [], sample_id
        return sample
